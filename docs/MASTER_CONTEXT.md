...
| 2026-08-21 | Invite org_admin: hosted Auth Site URL `https://smart-senior.pages.dev` (było `http://localhost:3000`). Secret `SITE_URL`. Redeploy `onboard-organization` — `redirectTo` zawsze `/logowanie` na Pages, nigdy loopback. |
| 2026-08-21 | Fix pętli logowania: middleware nie robi self-redirect `/logowanie`→`/logowanie` gdy JWT bez roli; callback `code`/`token_hash` bez przerzucania. |
